...
/**
 * Ordinal dispatch: string discriminators go through a `{value: ordinal}` table
 * and a dense integer switch, because V8 lowers a switch over STRING labels to
 * sequential `===` comparisons — 0.5 ns per preceding case, so 52 ns of pure
 * dispatch at 80 variants. The table keeps it flat (measured 1.9x at 8 variants,
 * 2.8x at 60) for ~1% more generated bytes.
 */
describe("fast-path — ordinal dispatch table", () => {
  ...
  it("dispatches 3+ string variants through the table", () => {
    const code = codeOf(variantIR(["a", "b", "c"]));
    expect(code).toContain('{"a":1,"b":2,"c":3}');
    expect(code).toContain("typeof __dv_");
  });

  it("keeps the plain string switch below the measured crossover", () => {
    const code = codeOf(variantIR(["a", "b"]));
    expect(code).not.toContain('{"a":1,"b":2}');
  ...
  it("rejects a non-string discriminator that would COERCE to a valid key", () => {
    // The matched case drops its own discriminator check, so an unguarded
    // `table[input.type]` would let `{toString(){return "alpha"}}` — which
    // strict equality never matched — select a case and pass.
  ...
  it("keeps the switch when a discriminator value is __proto__", () => {
    // An object literal cannot hold an own `__proto__` key, so the table would
    // silently lose that variant.
  ...
  it("gives values that select the same option one shared ordinal", () => {
    const ir: DiscriminatedUnionIR = {
      cases: [
        { option: 0, value: "a" },
        { option: 0, value: "b" },
    ...
    expect(codeOf(ir)).toContain('{"a":1,"b":1,"c":2}');
